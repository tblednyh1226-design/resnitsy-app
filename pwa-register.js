(()=>{
  ...
  async function hardRefreshIfNeeded(){
    try{
      ...
      if('caches' in window){
        ...
        await Promise.all(keys.filter(k=>k.startsWith('slotelly-pwa-')).map(k=>caches.delete(k)));
      }
      if('serviceWorker' in navigator){
        const regs=await navigator.serviceWorker.getRegistrations();
        await Promise.all(regs.map(async reg=>{ try{ await reg.update(); }catch(e){} }));
      }
      if(!location.search.includes('slotelly_refresh=')){
        const u=new URL(location.href);
        u.searchParams.set('slotelly_refresh',BUILD);
  ...
  function ensureInstallButton(){
    ...
    btn.id='slotellyInstallBtn'; btn.type='button'; btn.textContent='Установить Slotelly';
    ...
    btn.addEventListener('click',async()=>{
      if(deferredPrompt){ deferredPrompt.prompt(); try{await deferredPrompt.userChoice}catch(e){} deferredPrompt=null; btn.remove(); return; }
      const inApp=/wv|Telegram|Instagram|FBAN|FBAV/i.test(navigator.userAgent);
      alert(inApp?'Откройте эту страницу в Chrome через меню ⋮ → «Открыть в Chrome», затем нажмите «Установить Slotelly».':'В Chrome откройте меню ⋮ и выберите «Установить приложение» или «Добавить на главный экран».');
    });
  ...
  window.addEventListener('load',()=>{
    ...
    (async()=>{
      ...
      }catch(e){ console.warn('PWA registration failed',e); await hardRefreshIfNeeded(); }
    })();
  });
})();