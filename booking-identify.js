...
(function(){
  ...
  const s0=makeSection('s0','<div class="scroll"><h3>Давайте познакомимся</h3><div class="sub" style="margin-bottom:12px">Введите имя и номер телефона. По номеру мы только внутренне проверим, были ли вы у нас раньше.</div><div class="field"><label>Ваше имя</label><input id="identityName" autocomplete="name" placeholder="Как к вам обращаться?"></div><div class="field"><label>Телефон</label><input id="identityPhone" type="tel" autocomplete="tel" inputmode="tel" placeholder="+7 999 123-45-67"></div><div id="identityError"></div></div><div class="actions one"><button id="identityNext" class="btn primary">Далее</button></div>');
