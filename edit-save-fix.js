...
window.form = function(a=null, ds=focus, startTime='16:00') {
  ...
  function renderRows(){svcRowsEl.innerHTML='';rows.forEach((r,i)=>{const x=document.createElement('div');x.className='card';const active=services.filter(s=>s.is_active || s.id===r.service_id);x.innerHTML='<div class="field"><select class="ss">'+active.map(s=>'<option value="'+s.id+'" '+(s.id===r.service_id?'selected':'')+'>'+s.name+'</option>').join('')+'</select></div><div class="twocol"><div class="field"><label>Цена</label><input class="sp" type="number" value="'+Number(r.price||0)+'"></div><div class="field"><label>Длительность, мин</label><input class="sd" type="number" value="'+Number(r.duration||60)+'"></div></div>';const ss=x.querySelector('.ss'),sp=x.querySelector('.sp'),sd=x.querySelector('.sd');ss.addEventListener('change',()=>{const s=services.find(z=>z.id===ss.value);if(!s)return;rows[i]={service_id:s.id,name:s.name,standard_price:Number(s.base_price||0),price:Number(s.base_price||0),duration:Number(s.duration_minutes||60)};renderRows()});sp.addEventListener('input',()=>rows[i].price=Number(sp.value||0));sd.addEventListener('input',()=>rows[i].duration=Math.max(1,Number(sd.value||60)));svcRowsEl.appendChild(x)})}
  renderRows();
