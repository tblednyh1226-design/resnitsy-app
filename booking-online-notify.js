/* Send confirmation and auto-offer better times after successful online booking. */
(function(){
      ...
      if(url.includes('/functions/v1/resnitsy-booking')&&String(init?.method||'GET').toUpperCase()==='POST'){
        ...
        if(body.action==='book'&&r.ok){
          ...
          if(data?.ok&&data?.id){
            ...
            if(String(body.betterText||'').trim()){
              setTimeout(()=>originalFetch('https://acukaqoguzkrphauovhk.supabase.co/functions/v1/resnitsy-waitlist-autooffer',{method:'POST',headers:{'Content-Type':'text/plain;charset=UTF-8'},body:JSON.stringify({appointmentId:data.id}),cache:'no-store'}).catch(()=>{}),250);
            }
          }
        }
      }
